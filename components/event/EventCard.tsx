...
const CAT: Record<string, string> = { CONCERT:'콘서트', THEATER:'연극', MUSICAL:'뮤지컬', DANCE:'무용', EXHIBITION:'전시', SPORT:'스포츠' };
      ...
      <div className="card" style={{ cursor: 'pointer', opacity: isSoldOut ? 0.72 : 1 }}>
        ...
        <div style={{ position: 'relative', paddingBottom: '133%', overflow: 'hidden', background: 'var(--gray-200)' }}>
          ...
          <div style={{ position: 'absolute', top: 8, left: 8 }}>
            <span className="badge badge-navy" style={{ fontSize: '0.65rem' }}>{CAT[event.category] || event.category}</span>
          </div>
          {/* 상태 배지 */}
          {st.label && (
          ...
          {isSoldOut && (
            <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ background: 'rgba(0,0,0,0.7)', color: '#fff', fontSize: '0.82rem', fontWeight: 700, padding: '0.3rem 0.9rem', borderRadius: 100 }}>매진</span>
            </div>
          )}
        </div>
